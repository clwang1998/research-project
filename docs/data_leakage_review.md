...
```python
future_price = g[price_col].shift(-h)          # h ∈ [1, 5, 20, 30, ..., 90]
...
| `target_ret_fwd_90d` | 训练集最后 ~90 行（≈ 2018-08 之后） | 2019 年价格（验证期） |
...
| 90 | 42,946 | 2.146% | 43,926 | 17.967% |

### 风险等级

**高**。对于长 horizon（如 60d、90d），泄漏范围达数月，会系统性地高估模型在验证集上的表现。
...
原回测对所有 horizon 都按固定的 `--rebalance-every 5` 调仓，但每次调仓记的是完整 `target_ret_fwd_{h}d`（h 日前向收益）。当 `h > 5` 时，相邻调仓日的持有窗口高度重叠（例如 90D 目标每 5 天记一次 90 日收益，约 18 倍重叠）。同时 `summarize_ic` 用 `sqrt(252)` 对高度自相关的每日 IC 序列做年化。两者叠加使 Sharpe 与 ICIR 随 horizon 单调虚增，这是评估口径伪迹，而非真实 alpha。
