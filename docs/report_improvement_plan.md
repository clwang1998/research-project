...
两条叙事路线（选一条讲到底）：
- **路线 A（诚实但有 conviction）**：纯价量 + 当前成分股下，可稳健交易的 alpha 主要就是低换手横截面动量（5d sector Sharpe 约 0.63，10d/20d 约 0.75--0.82，且换手显著低于模型）；ML 增量 alpha 统计不显著（DSR/PBO 未过生产阈值）→ **决策：不上复杂模型，上低换手动量 + 严格成本/容量约束，下一笔研究预算投 point-in-time 宇宙**。
